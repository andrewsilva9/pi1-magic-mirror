...
/**
Copyright 2014-2016 Aleksandar Urosevic (urke.kg@gmail.com)
...
define( 'WPAU_STOCK_TICKER_VER', '0.2.0.1' );
	...
	class Wpau_Stock_Ticker
	{
		...
		public static function shortcode($atts) {
			...
			if ( ! empty( $symbols ) ) {
				...
				$empty = empty( $prefill ) ? 'true' : 'false';
				// duplicate
				$duplicate = empty( $duplicate ) ? 'false' : 'true';

				// Return stock holder
				return sprintf(
					'<div
					 class="stock-ticker-wrapper"
					 data-stockticker_symbols="%s"
					 data-stockticker_show="%s"
					 data-stockticker_static="%s"
					 data-stockticker_nolink="%s"
					 data-stockticker_class="%s"
					 data-stockticker_speed="%s"
					 data-stockticker_empty="%s"
					 data-stockticker_duplicate="%s"
					><ul class="stock_ticker"><li class="zero"><span class="sqitem">Loading stock data...</span></li></ul></div>',
					$symbols, $show, $static, $nolink, $class, $speed, $empty, $duplicate
				);
			}
			return false;
